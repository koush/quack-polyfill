...
class QuackEventLoop(val loop: AsyncEventLoop, val netLoop: AsyncEventLoop, val quack: QuackContext) {
    ...
    fun installXHR(rejectUnauthorized: Boolean = true): AsyncHttpClient {
        ...
        if (!rejectUnauthorized) {
            client.schemeExecutor.useInsecureHttpsExecutor(client.eventLoop)
        }
...
private fun SchemeExecutor.useInsecureHttpsExecutor(eventLoop: NIOEventLoop) =
        useHttpsExecutor(affinity, wrapConnectFirstAvailableResolver(createDefaultResolver(eventLoop)) { host, port ->
            val options = AsyncTlsOptions(object : HostnameVerifier {
                override fun verify(engine: SSLEngine): Boolean {
                    return true
                }
            }, null)
            connectTls(host, port, TlsModule.insecureContext, options)
        })
